...
class WPHScraper:
        ...
        try:
            ...
            for p in paragraphs:
                ...
                if len(text) > 50 and not any(skip in text.lower() for skip in [
                    'comment', 'subscribe', 'audiobook', 'itunes', 'podcast', 
            ...
            poker_terms = ['flop', 'turn', 'river', 'preflop', 'bet', 'raise', 'call', 'fold', 
                          'stack', 'pot', 'position', 'blind']
    ...
    def _extract_situation(self, text: str) -> str:
        ...
        for line in lines[:5]:
            if len(line) > 100:  # Substantial paragraph
                ...
                setup_indicators = ['flop', 'professional poker', 'cash game', 'tournament', 
                                  'stakes', 'blinds', 'high stakes', 'lodge', 'hustler']
    ...
    def _extract_strategic_analysis(self, text: str) -> str:
        ...
        strategic_indicators = [
            'when you', 'you should', 'you need to', 'generally', 'optimal', 
            'correct play', 'best', 'strategy', 'equity', 'pot odds', 'range',
            'value bet', 'bluff', 'position', 'stack to pot ratio', 'outs'
        ]
        
        for para in paragraphs:
            if len(para) > 100:  # Substantial paragraph
                # Count strategic terms in paragraph
                strategic_count = sum(1 for term in strategic_indicators if term in para.lower())
    ...
    def _extract_key_learnings(self, text: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            skip_terms = ['comment', 'question', 'subscribe', 'download', 'free', 'audiobook']
            if any(skip in sentence.lower() for skip in skip_terms):
                continue
            
            # Look for instructional content
            learning_indicators = [
                'you should', 'you need to', 'always', 'never', 'generally',
                'important to', 'key is', 'remember', 'when facing', 'if you have'
            ]
        ...
        principle_sentences = []
        for sentence in sentences:
            if any(term in sentence.lower() for term in ['equity', 'position', 'pot odds', 'value', 'bluff']):
    ...
    def save_to_json(self, hands: List[PokerHand], filename: str):
        ...
        summary = {
            ...
            "titles": [f"Episode {hand.episode_number}: {hand.title}" for hand in hands],
            "sample_hand": asdict(hands[0]) if hands else None
        }
        
        # Use episode range in summary filename too
        if hands:
            summary_filename = f"data/wph_scrape_summary_{min_episode}_{max_episode}.json"
        else:

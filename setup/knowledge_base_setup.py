...
class KnowledgeBaseSetup:
    ...
    def initialize_knowledge_base(
        self,
    ) -> Tuple[Optional[PokerKnowledgeBase], Dict[str, Any]]:
        """
        ...
        try:
            ...
            print(
                "This will scrape real Jonathan Little WPH episodes - may take a few minutes"
            )
            ...
            if data_files_info["has_data"]:
                print(
                    f"📁 Found {data_files_info['total_files']} existing data files on disk:"
                )
            ...
            if stats.get("hands_indexed", 0) == 0:
                ...
                print(
                    "⏱️ Please be patient - this respects rate limits and may take 2-3 minutes"
                )
                ...
                if hands and len(hands) > 0:
                    ...
                    setup_info["message"] = (
                        f"Successfully created knowledge base with {len(hands)} hands"
                    )

                    print("\n✅ SUCCESS! Knowledge base created with:")
                    print(f"   📚 {final_stats.get('hands_indexed', 0)} hand analyses")
                    print(
                        f"   💡 {final_stats.get('strategies_indexed', 0)} strategic insights"
                    )
                    print(
                        f"   📄 {final_stats.get('total_documents', 0)} total documents"
                    )
                    ...
                    raise Exception(
                        "Scraping completed but no hands were successfully processed"
                    )
            else:
                setup_info["success"] = True
                setup_info["message"] = (
                    f"Existing knowledge base loaded with {stats.get('total_documents', 0)} documents"
                )
            ...
            print(
                "   • Try running the setup module standalone: python setup/knowledge_base_setup.py"
            )

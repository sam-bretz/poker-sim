...
- Bet sizing affects required success rate
    ...
    def calculate_implied_odds(self, pot_size: float, bet_to_call: float, 
                             implied_winnings: float, equity: float) -> Dict[str, Any]:
    ...
    def estimate_hand_equity(self, hole_cards: str, board: str = "", opponents: int = 1) -> Dict[str, Any]:
    ...
    def analyze_betting_decision(self, situation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if bet_to_call > 0:
            ...
            raise_size = max(bet_to_call * 2, pot_size * 0.5)
    ...
    def get_recommendation(self, situation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'pot_odds' in analysis:
            ...
            reasoning_parts.append(f"Pot odds: {pot_odds['getting_odds']} ({required_equity}% equity needed)")
            reasoning_parts.append(f"Estimated equity: {equity}%")
            
            if equity > required_equity:
                recommendation = f"CALL - Equity ({equity}%) > Required ({required_equity}%)"
                confidence = 0.8
                reasoning_parts.append("✓ Profitable call based on pot odds")
            else:
                recommendation = f"FOLD - Equity ({equity}%) < Required ({required_equity}%)"
                confidence = 0.8
        ...
        if 'expected_values' in analysis:
            ...
            reasoning_parts.append(f"Best EV action: {best_action[0]} (+{best_action[1]})")
            
            if best_action[1] > 0:
                recommendation = f"{best_action[0].upper()} - Best EV (+{best_action[1]})"
                confidence = 0.7
        
        reasoning = "\n".join(reasoning_parts) if reasoning_parts else "Mathematical analysis incomplete"

...
Always provide context from the knowledge base when available, and explain how the strategic concepts apply to the current situation.
    ...
    def apply_jonathan_framework(self, situation: Dict[str, Any], context: str) -> Dict[str, str]:
        ...
        if position in ['btn', 'button', 'co', 'cutoff']:
            framework_analysis["range_analysis"] = "Late position allows for wider opening ranges and more bluffing opportunities"
        elif position in ['utg', 'ep']:
            framework_analysis["range_analysis"] = "Early position requires tight ranges and strong hands"
        elif position in ['bb', 'sb']:
            framework_analysis["range_analysis"] = "Blind positions require careful defense frequency calculations"
        ...
        if situation.get('pot_size', 0) > 0:
            ...
            if situation.get('bet_to_call', 0) > 0:
                ...
                if bet_ratio < 0.5:
                    framework_analysis["bet_sizing"] = "Small bet size suggests value betting or pot control"
                elif bet_ratio > 1.0:
                    framework_analysis["bet_sizing"] = "Large bet size indicates strong hand or big bluff"
                else:
                    framework_analysis["bet_sizing"] = "Standard bet sizing, analyze based on range and position"
        
        # GTO vs Exploitative decision
        if 'aggressive' in context.lower() or 'loose' in context.lower():
            framework_analysis["gto_vs_exploitative"] = "Opponent appears loose/aggressive - tighten up and value bet more"
        elif 'tight' in context.lower() or 'passive' in context.lower():
            framework_analysis["gto_vs_exploitative"] = "Opponent appears tight/passive - can bluff more and value bet thinner"
    ...
    def get_recommendation(self, situation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if strategic_context and len(strategic_context) > 50:
            ...
            reasoning_parts.append(strategic_context[:400] + "..." if len(strategic_context) > 400 else strategic_context)
            confidence = 0.8
        ...
        if situation.get('hole_cards') and situation.get('position'):
            ...
            if hole_cards.upper() in ['AA', 'KK', 'QQ', 'AK']:
                recommendation = f"Premium hand ({hole_cards}) - bet for value, build pot"
                confidence = 0.9
            elif 'btn' in position or 'co' in position:
                recommendation = f"In position with {hole_cards} - can play more aggressively"
                confidence = 0.8
            elif position in ['utg', 'ep'] and hole_cards.upper() not in ['AA', 'KK', 'QQ', 'JJ', 'AK']:
                recommendation = f"Early position with {hole_cards} - consider tighter play"
                confidence = 0.8
        
        # Check if we have betting action
        if situation.get('bet_to_call', 0) > 0:
            pot_odds_needed = situation['bet_to_call'] / (situation.get('pot_size', 0) + situation['bet_to_call']) * 100
        ...
        return {
            ...
            "reasoning": reasoning,
            "strategic_context": strategic_context[:200] + "..." if len(strategic_context) > 200 else strategic_context,
